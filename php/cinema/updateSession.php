<?php
    ...
    $data = json_decode(file_get_contents('php://input'),true);
        ...
        $sql_SELECT = "SELECT  `datetime`, `id_hall`, `id_film`, `timing`
                        ...
                        WHERE `id_hall` = ? AND DATE(`datetime`) = DATE(?) AND (`sessions`.`id`<>?)";
        $stmt_SELECT = mysqli_prepare($mysqli, $sql_SELECT);
        mysqli_stmt_bind_param($stmt_SELECT, 'isi', $hall, $date,$idSession);
        ...
        if ($sessions == null){
            $out['add'] = 'Session net'; 
                $sql_INSERT = "
                                UPDATE `sessions` 
                                SET `datetime`=?,`id_hall`=?,`id_price`=?,`movieFormat`=? WHERE `id`=?";
                $stmt_INSERT = mysqli_prepare($mysqli, $sql_INSERT);
                mysqli_stmt_bind_param($stmt_INSERT, 'siisi', 
                                        $date, $hall, $price, 
                                        $format, $idSession);
